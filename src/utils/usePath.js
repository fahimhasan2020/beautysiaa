...
const usePath = () => {
  ...
  const containerPath = useMemo(() => {
    return `M0,0L${sizes.width},0L${sizes.width},${tHeight}L0,${tHeight}L0,0`;
  }, [tHeight]);

  const curvedPaths = useMemo(() => {
    return Array.from({ length: NUM_TABS }, (_, index) => {
      const tabShapePath = generateTabShapePath(index + 0.5, adjustedHeight, NUM_TABS);
      return parse(`${tabShapePath}`);
    });
  }, [adjustedHeight, NUM_TABS]);
